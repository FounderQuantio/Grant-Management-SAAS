...
def create_app() -> FastAPI:
    ...
    @app.post("/api/v1/diag/apply-migration-v2007", include_in_schema=False)
    async def apply_migration_v2007(user=core_auth.RequireSystemAdmin) -> dict:
        """TEMPORARY — applies database/migrations/v2_007_notifications.sql
        statement-by-statement. Remove after use."""
        import pathlib
        from core.db import engine
        from sqlalchemy import text as sql_text

        path = pathlib.Path(__file__).parent / "database" / "migrations" / "v2_007_notifications.sql"
        raw = path.read_text()
